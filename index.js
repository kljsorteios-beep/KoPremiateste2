...
async function updateTicketDocuments(numbers, fields) {
  if (!numbers?.length) return;
  const writer = db.bulkWriter();
  const operations = numbers.map((ticketNumber) => {
    const ticketRef = db.doc(`cotas/${parseTicketNumber(ticketNumber)}`);
    return writer.set(ticketRef, {
      numero: parseTicketNumber(ticketNumber),
      ...fields,
...
async function confirmPaidOrder(orderId, payload) {
  ...
  if (paid && numbersToMark.length) {
    await updateTicketDocuments(numbersToMark, {
      status: 'vendida',
      pedidoId: orderId,
...
exports.createPixOrder = onCall({
  ...
  try {
    ...
    await updateTicketDocuments(selectedNumbers, {
      status: 'reservada',
      pedidoId: orderId,
